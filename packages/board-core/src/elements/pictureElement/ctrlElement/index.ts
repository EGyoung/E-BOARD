...
// 矩形元素控制类
class PictureCtrlElement extends BaseCtrlElement {
    protected static configService: IConfigService | null = null;

    // 只在第一次使用时去 Container 查找一次
    protected get configService() {
        if (!PictureCtrlElement.configService) {
            PictureCtrlElement.configService = eBoardContainer.get<IConfigService>(IConfigService);
        }
        return PictureCtrlElement.configService;
    }
